...
StardustASSEGVendorLogic = ScreenPlay:new {
	scriptName = "StardustASSEGVendorLogic",
	--EXAMPLE SETUP
	currencies = {
...
registerScreenPlay("StardustASSEGVendorLogic", false)

function StardustASSEGVendorLogic:openSUIArmorsmithSegments(pCreatureObject, pUsingObject)
...
function StardustASSEGVendorLogic:defaultCallbackArmorsmithSegments(pPlayer, pSui, eventIndex, args)
...
function StardustASSEGVendorLogic:buyItemArmorsmithSegments(pPlayer, itemSelected)
...
function StardustASSEGVendorLogic:getMerchandiseStringArmorsmithSegments(num)
...
function StardustASSEGVendorLogic:hasEnoughCurrencyArmorsmithSegments(pPlayer, num)
	local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
	local containerSize = SceneObject(pInventory):getContainerObjectsSize()
	local pGhost = CreatureObject(pPlayer):getPlayerObject()
...
function StardustASSEGVendorLogic:payTokensArmorsmithSegments(pPlayer, selectedItem, num)
	local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
	local containerSize = SceneObject(pInventory):getContainerObjectsSize()
	local pGhost = CreatureObject(pPlayer):getPlayerObject()
