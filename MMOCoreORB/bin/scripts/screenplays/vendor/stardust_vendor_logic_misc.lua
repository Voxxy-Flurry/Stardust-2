...
StardustMiscellaneousVendorLogic = ScreenPlay:new {
	scriptName = "StardustMiscellaneousVendorLogic",
	--EXAMPLE SETUP
	currencies = {
		...
		--Displayed Name, 
		merchandise_misc = { -- Displayed name, full template string (without the shared_), cost {} - follow same order as the currencies setup previously
...
registerScreenPlay("StardustMiscellaneousVendorLogic", false)

function StardustMiscellaneousVendorLogic:openSUIMiscellaneous(pCreatureObject, pUsingObject)
	local sui = SuiListBox.new(self.scriptName, "defaultCallbackMiscellaneous")
	...
	for i = 1, #self.merchandise_misc, 1 do
		local merchString = self:getMerchandiseStringMiscellaneous(i)
		sui.add(merchString, "")
	end
...
function StardustMiscellaneousVendorLogic:defaultCallbackMiscellaneous(pPlayer, pSui, eventIndex, args)
	...
	self:buyItemMiscellaneous(pPlayer, selectedOption)
end

function StardustMiscellaneousVendorLogic:buyItemMiscellaneous(pPlayer, itemSelected)
	local merch = self.merchandise_misc[itemSelected]
	...
	local canPurchase = self:hasEnoughCurrencyMiscellaneous(pPlayer, itemSelected)
		...
		for i = 1, #merch.cost do
			...
			if (cost ~= 0) then
				if (currency == "token") then
					self:payTokensMiscellaneous(pPlayer, itemSelected, i)
				end
...
function StardustMiscellaneousVendorLogic:getMerchandiseStringMiscellaneous(num)
	local merch = self.merchandise_misc[num]
...
function StardustMiscellaneousVendorLogic:hasEnoughCurrencyMiscellaneous(pPlayer, num)
	local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
	local containerSize = SceneObject(pInventory):getContainerObjectsSize()
	local pGhost = CreatureObject(pPlayer):getPlayerObject()
...
function StardustMiscellaneousVendorLogic:payTokensMiscellaneous(pPlayer, selectedItem, num)
	local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
	local containerSize = SceneObject(pInventory):getContainerObjectsSize()
	local pGhost = CreatureObject(pPlayer):getPlayerObject()
	
	local merch = self.merchandise_misc[selectedItem]
